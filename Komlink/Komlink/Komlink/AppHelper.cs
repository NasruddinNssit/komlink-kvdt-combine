using Komlink.Base.LocalDevices;
        ...
        public bool SystemHealthCheck()
        {
            ...
            bool HealthCheckWorking()
            {
                    ...
                    && (serverStatusAck?.IsResultSuccess == true) && (serverStatusAck.MessageData is CounterConfigCompiledResult configRes))
                {
                    ...
                    {
                       Log.Error(_logChannel, "-", ex, "EX02", "AppHelper.SystemHealthCheck");
                    ...
                    if (isPrinterError || isPrinterWarning)
                    {
                       ...
                       Log.Information(_logChannel, "-", $@"Error; (EXIT10000018); {statusDescription}", "A11A", "AppHelper.SystemHealthCheck", NssIT.Kiosk.AppDecorator.Log.MessageType.Error);
                    ...
                    if (NssIT.Kiosk.AppDecorator.Config.Setting.GetSetting().DisablePrinterTracking == false)
                    {
                        ...
                        {
                            Log.Information(_logChannel, "-", $@"Error; (EXIT10000019); Default (or correct) printer not found", "A11B", "AppHelper.SystemHealthCheck", NssIT.Kiosk.AppDecorator.Log.MessageType.Error);
                ...
                {
                    Log.Information(_logChannel, "-", serverStatusAck, "A20", "AppHelper.SystemHealthCheck", NssIT.Kiosk.AppDecorator.Log.MessageType.Error,
                       $@"Error; (EXIT10000010); isServerResponded: {isServerResponded}; serverAppHasDisposed: {serverAppHasDisposed}; serverAppHasShutdown: {serverAppHasShutdown}; MsgObj: UIServerApplicationStatusAck");
