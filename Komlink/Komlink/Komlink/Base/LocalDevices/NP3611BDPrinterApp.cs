...
namespace Komlink.Base.LocalDevices
{
    public class NP3611BDPrinterApp : IDisposable 
    {
        ...
        public static NP3611BDPrinterApp GetApp(string printerName, bool checkPrinterPaperLowState)
        {
            ...
            catch (Exception ex)
            {
                ...
                Log.Error(_logChannel, "*", nEx, "EX01", "NP3611BDPrinterApp.GetApp");
                throw nEx;
            }
            finally
            {
        ...
        private NP3611BDPrinterApp(string initPrinterName, bool checkPrinterPaperLowState)
        {
            ...
            Log.Information(_logChannel, "*", $@"Parameter_Printer_Name : {initPrinterName ?? ""} ; Check_Printer_Paper_Low_State : {checkPrinterPaperLowState}", "A01", "NP3611BDPrinterApp.<Constructor>", NssIT.Kiosk.AppDecorator.Log.MessageType.Info);
        }
        ...
        public void GetPrinterStatus(out bool isPrinterError, out bool isPrinterWarning, out string statusDescription, out string localStatusDescription)
        {
            ...
            if (NssIT.Kiosk.AppDecorator.Config.Setting.GetSetting().DisablePrinterTracking)
            {
            ...
            {
                Log.Information(_logChannel, "*", msg, "A05", "NP3611BDPrinterApp.GetPrinterStatus", NssIT.Kiosk.AppDecorator.Log.MessageType.Info);
