...
{
    public class TnGCardInfoResp : ICardResponse
    {
        /// <summary>
        /// Importance
        /// </summary>
        ...
        public string TKCardSerialNo { get; private set; } = "";
        public string TnGIdentificationCode { get; private set; } = "";
        public string TnGCompanyCode { get; private set; } = "";
        public string TnGCode1 { get; private set; } = "";
        public string TnGAccountCode { get; private set; } = "";
        public string TnGLuhnKey { get; private set; } = "";

        public decimal TKMainPurseAmount { get; private set; } = 0;
        public uint TKMainTransNo { get; private set; } = 0;
        public int TKBlacklistFlag { get; private set; } = 0;
        ...
        public TnGCardInfoResp(IM30DataModel orgData)
        {
            if (orgData is null)
            ...
            if ((DataError is null) && ((TransactionCodeDef.IsEqualTrans(orgData.TransactionCode, TransactionCodeDef.StartTransaction) == false)))
            {
                ...
                {
                    DataError = new Exception("-Touch n Go card related info not found~");
                }
            }
            ...
            if (DataError is null)
            {
                ...
                /////-----------------------------------------------------------------------------------------------------------
                {
                    IM30FieldElementModel cdT0 = (from fe in orgData.FieldElementCollection
                                                  where FieldTypeDef.IsEqualType(fe.FieldTypeCode, FieldTypeDef.TKCardSerialNo)
                                                  select fe).FirstOrDefault();

                    if ((cdT0 is null) || (string.IsNullOrWhiteSpace(cdT0.Data)))
                    {
                        errMsg += "Invalid TnG Card Serial No.;";
                    }
                    else
                        TKCardSerialNo = cdT0.Data.Trim();
                }
                /////-----------------------------------------------------------------------------------------------------------
                {
                    IM30FieldElementModel cdT1 = (from fe in orgData.FieldElementCollection
                                                  where FieldTypeDef.IsEqualType(fe.FieldTypeCode, FieldTypeDef.TnGIdentificationCode)
                                                  select fe).FirstOrDefault();

                    if ((cdT1 is null) || (string.IsNullOrWhiteSpace(cdT1.Data)))
                    {
                        errMsg += "Invalid TnG Identification Code;";
                    }
                    else
                        TnGIdentificationCode = cdT1.Data.Trim();
                }
                /////-----------------------------------------------------------------------------------------------------------
                {
                    IM30FieldElementModel cdT2 = (from fe in orgData.FieldElementCollection
                                                  where FieldTypeDef.IsEqualType(fe.FieldTypeCode, FieldTypeDef.TnGCompanyCode)
                                                  select fe).FirstOrDefault();

                    if ((cdT2 is null) || (string.IsNullOrWhiteSpace(cdT2.Data)))
                    {
                        errMsg += "Invalid TnG Company Code;";
                    }
                    else
                        TnGCompanyCode = cdT2.Data.Trim();
                }
                /////-----------------------------------------------------------------------------------------------------------
                {
                    IM30FieldElementModel cdT3 = (from fe in orgData.FieldElementCollection
                                                  where FieldTypeDef.IsEqualType(fe.FieldTypeCode, FieldTypeDef.TnGCode1)
                                                  select fe).FirstOrDefault();

                    if ((cdT3 is null) || (string.IsNullOrWhiteSpace(cdT3.Data)))
                    {
                        errMsg += "Invalid TnG Code1;";
                    }
                    else
                        TnGCode1 = cdT3.Data.Trim();
                }
                /////-----------------------------------------------------------------------------------------------------------
                {
                    ...
                    if ((cdT4 is null) || (string.IsNullOrWhiteSpace(cdT4.Data)))
                    {
                        errMsg += "Invalid TnG Account Code;";
                    }
                    else
                        TnGAccountCode = cdT4.Data.Trim();
                }
                /////-----------------------------------------------------------------------------------------------------------
                {
                    IM30FieldElementModel cdT5 = (from fe in orgData.FieldElementCollection
                                                  where FieldTypeDef.IsEqualType(fe.FieldTypeCode, FieldTypeDef.TnGLuhnKey)
                                                  select fe).FirstOrDefault();

                    if ((cdT5 is null) || (string.IsNullOrWhiteSpace(cdT5.Data)))
                    {
                        errMsg += "TnG Luhn Key not found;";
                    }
                    else
                        TnGLuhnKey = cdT5.Data.Trim();
                }
                /////-----------------------------------------------------------------------------------------------------------
                {
                ...
                {
                    IM30FieldElementModel cdT6 = (from fe in orgData.FieldElementCollection
                                                  where FieldTypeDef.IsEqualType(fe.FieldTypeCode, FieldTypeDef.TKBlacklistFlag)
                                                  select fe).FirstOrDefault();

                    if ((cdT6 is null) || (string.IsNullOrWhiteSpace(cdT6.Data)))
                        errMsg += "TnG Blacklist Flag not found;";

                    else if (int.TryParse(cdT6.Data.Trim(), out int bklFlag))
                    {
                        TKBlacklistFlag = bklFlag;
                    }
                    else
                        errMsg += $@"-Invalid TnG Blacklist Flag~({cdT6.Data});";
                }
                /////-----------------------------------------------------------------------------------------------------------
                {
