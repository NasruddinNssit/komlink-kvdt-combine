...
{
    public class CreditDebitChargeCardResp : ICardResponse
    {
        /// <summary>
        /// Importance
        /// </summary>
        ...
        public CreditDebitChargeCardResp(IM30DataModel orgData)
        {
            if (orgData is null)
            ...
            if ((DataError is null) && ((TransactionCodeDef.IsEqualTrans(orgData.TransactionCode, TransactionCodeDef.ChargeAmount) == false)))
            {
                ...
                {
                    DataError = new Exception("-Credit/Debit card related info not found~");
                }
            }
            ...
            if (DataError is null)
            {
                ...
                /////-----------------------------------------------------------------------------------------------------------
                {
                    IM30FieldElementModel cdE0 = (from fe in orgData.FieldElementCollection
                                                  where FieldTypeDef.IsEqualType(fe.FieldTypeCode, FieldTypeDef.AID)
                                                  select fe).FirstOrDefault();

                    if ((cdE0 is null) || (string.IsNullOrWhiteSpace(cdE0.Data)))
                    {
                        //Not Importance .. errMsg += "Invalid Application ID;";
                    }
                    else
                        AID = cdE0.Data.Trim();
                }
                /////-----------------------------------------------------------------------------------------------------------
                {
                    IM30FieldElementModel cdE1 = (from fe in orgData.FieldElementCollection
                                                  where FieldTypeDef.IsEqualType(fe.FieldTypeCode, FieldTypeDef.ApplicationName)
                                                  select fe).FirstOrDefault();

                    if ((cdE1 is null) || (string.IsNullOrWhiteSpace(cdE1.Data)))
                    {
                        //Not Importance .. errMsg += "Invalid Application Name;";
                    }
                    else
                        ApplicationName = cdE1.Data.Trim();
                }
                /////-----------------------------------------------------------------------------------------------------------
                {
                    IM30FieldElementModel cdE2 = (from fe in orgData.FieldElementCollection
                                                  where FieldTypeDef.IsEqualType(fe.FieldTypeCode, FieldTypeDef.Cryptogram)
                                                  select fe).FirstOrDefault();

                    if ((cdE2 is null) || (string.IsNullOrWhiteSpace(cdE2.Data)))
                    {
                        errMsg += "Invalid Cryptogram;";
                    }
                    else
                        Cryptogram = cdE2.Data.Trim();
                }
                /////-----------------------------------------------------------------------------------------------------------
                {
                    IM30FieldElementModel cdE3 = (from fe in orgData.FieldElementCollection
                                                  where FieldTypeDef.IsEqualType(fe.FieldTypeCode, FieldTypeDef.TVR)
                                                  select fe).FirstOrDefault();

                    if ((cdE3 is null) || (string.IsNullOrWhiteSpace(cdE3.Data)))
                    {
                        //Not Importance .. errMsg += "Invalid Transaction verification result (TVR);";
                    }
                    else
                        TVR = cdE3.Data.Trim();
                }
                /////-----------------------------------------------------------------------------------------------------------
                {
                    IM30FieldElementModel cdE4 = (from fe in orgData.FieldElementCollection
                                                  where FieldTypeDef.IsEqualType(fe.FieldTypeCode, FieldTypeDef.CardVerificationMethod)
                                                  select fe).FirstOrDefault();

                    if ((cdE4 is null) || (string.IsNullOrWhiteSpace(cdE4.Data)))
                    {
                        //Not Importance .. errMsg += "Invalid Card Verification Method (CVM);";
                    }
                    else
                        CVM = cdE4.Data.Trim();
                }
        ...
        public CreditDebitChargeCardResp(Exception error)
        {
            IsDataFound = false;
