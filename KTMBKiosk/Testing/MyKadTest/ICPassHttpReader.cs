        ...
        public PassengerIdentity ReadIC(int waitDelaySec = 10)
        {
            ...
            bool ReadICDataToString(out string dataString, out string errorMessage, int waitDelayInSec = 10)
            {
                ...
                try
                {
                    ...
                    {
                        client.TimeoutMilliSec = 20 * 1000;
                        string downloadedString = client.DownloadString(_icReadingURL);

                        if (string.IsNullOrWhiteSpace(downloadedString) == false)
                        {
                            _msg.ShowMessage($@"ICPassHttpReader --> downloadedString: {downloadedString}");

                            dataString = downloadedString;
                            return true;
                        }
                        else
                        {
