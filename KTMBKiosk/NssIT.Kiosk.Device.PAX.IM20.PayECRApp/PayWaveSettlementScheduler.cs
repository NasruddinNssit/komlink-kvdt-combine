...
namespace NssIT.Kiosk.Device.PAX.IM20.PayECRApp
{
	public class PayWaveSettlementScheduler : IDisposable
	{
						...
						)
					{
						...
						try
						{
							...
							else
							{
								...
								int retryCount = 0, maxTry = 3, maxRetryWaitingDelaySec = 120;
								numberOfHost = 0;

								do
								{
									retList = _requestOutstandingSettlementInfoHandle(out isRequestSuccessful);
									...
									if (isRequestSuccessful == false)
                                    {
                                        ...
                                        {
											retryCount++;
											Thread.Sleep(maxRetryWaitingDelaySec * 100);
										}
									}

								} while (isRequestSuccessful == false);

								if (isRequestSuccessful)
								{
									foreach (string tmpHsno in retList)
									{
										_rebootRequestAfterSettlement = true;
										outStandingList.Add(tmpHsno);
										numberOfHost++;
									}
								}
				...
				finally
				{
					if (_startScheduleWorking)
					{
						...
						else
						{
							...
							//Wait 3 minutes to allow IM20 to Update Terminal Application (download latest app from Paysys) after finished settlement
		...
		private void SettlementCompletedCallBack(object sender, TrxCallBackEventArgs e)
		{
			if (!e.IsSuccess)
			{
				if (!(e.Result.StatusCode.Equals(ResponseStatusCode.BU) 
						|| e.Result.StatusCode.Equals(ResponseStatusCode.HE)
						|| e.Result.StatusCode.Equals(ResponseStatusCode.ZE)))
				{
					string failMsg = "";
					...
					if ((e.Result.HostNo != null) && (!e.Result.HostNo.Equals("99")))
					{
