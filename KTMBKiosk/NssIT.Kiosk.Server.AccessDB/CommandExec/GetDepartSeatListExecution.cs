...
namespace NssIT.Kiosk.Server.AccessDB.CommandExec
{
    public class GetDepartSeatListExecution : IAccessCommandExec, IDisposable
    {
        ...
        {
            string methodTag = "GetDepartSeatListExecution.Execute";
            string domainEntityTag = "Depart Seat List";
            ...
            try
            {
                ...
                if ((apiRes is TrainSeatResult seatX) && (seatX.Code.Equals(WebAPIAgent.ApiCodeOK)))
                {
                    ...
                    if ((seat?.Data?.ErrorCode == 0))
                    {
                        ...
                        uiSeatList = new UIDepartSeatListAck(commPack.NetProcessId, commPack.ProcessId, DateTime.Now, seat);
                        ...
                        apiRes = new WebApiException(new Exception($@"No valid data found; (EXIT21635); Error Code : {seat?.Data?.ErrorCode}"));
                }
                ...
                if (validDataFound == false)
                {
                    ...
                    {
                        errCode = "99";
                        errorMsg = $@"No valid data ({domainEntityTag}) found; Code: {errCode}; (EXIT21636)";
                    ...
                    {
                        errCode = wex.Code;
                        errorMsg = $@"{wex.MessageString() ?? "Web process error"}; ({domainEntityTag});Code: {errCode}; (EXIT21637)";
                    ...
                    {
                        errCode = "99";
                        errorMsg = $@"Unexpected error occurred; ({domainEntityTag}); Code: {errCode}; (EXIT21638)";
                    ...
                    uiSeatList = new UIDepartSeatListAck(commPack.NetProcessId, commPack.ProcessId, DateTime.Now, errData)
                    { ErrorMessage = (string.IsNullOrWhiteSpace(errorMsg) == true ? "Unknown error when reading data (EXIT21639)" : errorMsg) };
            ...
            {
                List<string> msgs = new List<string>(new string[] { $@"{ex.Message}; (EXIT21639A)" });
                TrainSeatResult errData = new TrainSeatResult() { Code = "99", Data = null, Messages = msgs, Status = false };

                uiSeatList = new UIDepartSeatListAck(commPack.NetProcessId, commPack.ProcessId, DateTime.Now, errData) { ErrorMessage = errData.MessageString() };
