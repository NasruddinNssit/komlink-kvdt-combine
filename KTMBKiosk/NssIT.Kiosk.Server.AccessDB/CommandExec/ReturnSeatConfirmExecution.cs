...
{
    public class ReturnSeatConfirmExecution : IAccessCommandExec, IDisposable
    {
        ...
        {
            string methodTag = "ReturnSeatConfirmExecution.Execute";
            string domainEntityTag = "Seat Booking";

            _serverAccess = serverAccess;
            _commandPack = commPack;
            bool eventSent = false;
            ReturnSeatConfirmCommand command = (ReturnSeatConfirmCommand)_commandPack.Command;
            UIReturnSeatConfirmResult uiSeatConf = null;
            ...
            {   // BookingResult
                ...
                if ((apiRes is BookingResult bookR) && (bookR.Code.Equals(WebAPIAgent.ApiCodeOK)) && (bookR.Data != null) && (bookR.Data.ErrorCode == 0))
                {
                    ...
                    {
                        uiSeatConf = new UIReturnSeatConfirmResult(commPack.NetProcessId, commPack.ProcessId, DateTime.Now, bookR);
                    ...
                    {
                        apiRes = new WebApiException(new Exception($@"No valid data found; {bookR.Data.ErrorMessage}; (EXIT21621)"));
                    }
                }
                ...
                if (validDataFound == false)
                {
                    ...
                    {
                        errCode = "99";
                        errorMsg = $@"No valid data ({domainEntityTag}) found; Code: {errCode}; (EXIT21622)";
                    ...
                    {
                        errCode = wex.Code;
                        errorMsg = $@"{wex.MessageString() ?? "Web process error"}; ({domainEntityTag});Code: {errCode}; (EXIT21623)";
                    ...
                    {
                        errCode = "99";
                        errorMsg = $@"Unexpected error occurred; ({domainEntityTag}); Code: {errCode};(EXIT21624)";
                    ...
                    uiSeatConf = new UIReturnSeatConfirmResult(commPack.NetProcessId, commPack.ProcessId, DateTime.Now, errData)
                    { ErrorMessage = (string.IsNullOrWhiteSpace(errorMsg) == true ? "Unknown error when reading data (EXIT21625)" : errorMsg) };
            ...
            {
                BookingResult errData = new BookingResult() { Code = "99", Data = null, Messages = new List<string>(new string[1] { $@"{ex.Message}; (EXIT21626)" }), Status = false };
                uiSeatConf = new UIReturnSeatConfirmResult(commPack.NetProcessId, commPack.ProcessId, DateTime.Now, errData) { ErrorMessage = $@"{ex.Message}; (EXIT21627)" };
