...
namespace NssIT.Kiosk.Device.PAX.IM30.OrgAPI.Trans.GetDeviceInfo
{
    public class IM30GetDeviceInfo : IIM30Trans, IDisposable
    {
        ...
        private GetDeviceInfoState _processState = GetDeviceInfoState.New;
        ...
        private List<GetDeviceInfoState> _processEndingStatesList = new List<GetDeviceInfoState>(new GetDeviceInfoState[]
            {GetDeviceInfoState.Busy, GetDeviceInfoState.Ending, GetDeviceInfoState.ErrorHalt, GetDeviceInfoState.Timeout });
        ...
        public static IM30GetDeviceInfo LastIM30GetDeviceInfoObj { get; private set; } = null;
        ...
        public IM30GetDeviceInfo(string comPort, string transactionID,
            ...
            ShowMessageLogDelg debugShowMessageDelgHandler = null)
        {
            if (string.IsNullOrWhiteSpace(comPort))
                throw new Exception("Invalid COM port specification when getting device info from reader");
        ...
        public IM30DataModel GetNewIM30Data()
        {
            IM30DataModel msgData = IM30RequestResponseDataWorks.CreateNewMessageData
                        (RequestResponseIndicatorDef.RequestAndResponse, TransactionCodeDef.GetDeviceInfo, ResponseCodeDef.Approved, MoreIndicatorDef.LastMessage);
        ...
        {
            return new IM30GetDeviceInfoResult(new Exception(errorMessage), null);
        }
        ...
        public bool StartTransaction(out Exception error)
        {
            ...
            if (IsDisposed)
            {
                error = new Exception("-Card reader Getting Device Info command already disposed~");
                return false;
            }
            else if (_processState != GetDeviceInfoState.New)
            {
                if (IsEndingProcess || IsCurrentWorkingEnded)
                {
                    error = new Exception($@"-Card reader Getting Device Info error. Getting Device Info command already finished~ Process ID : {_transactionID}");
                }
                else
                {
                    error = new Exception($@"-Card reader Getting Device Info error. Getting Device Info command has already started~ Process ID : {_transactionID}");
                }
                return false;
            }
            //--------------------------------------------------------------------------------------------------------------------------------

            UpdateProcessState(GetDeviceInfoState.Start, "-Start (S01)~");
            ...
            Thread tWorker = new Thread(new ThreadStart(new Action(() =>
            {
                ...
                try
                {
                    ...
                    Log.LogText(_logChannel, _transactionID, $@"COM Port: {COMPort}; --------- Start Card Reader COM Port Sequences : Get Device Info ---------", "K01", "IM30GetDeviceInfo.StartTransaction");

                    IM30DataModel dataModel = GetNewIM30Data();

                    LogState("-- Command --\r\n" + JsonConvert.SerializeObject(dataModel, Formatting.Indented));
                    Log.LogText(_logChannel, _transactionID, dataModel, $@"K03#COM Port: {COMPort}", "IM30GetDeviceInfo.StartTransaction");
                    ...
                    using (IM30COMPort im30Port = new IM30COMPort(COMPort, "-Getting Reader Info~", _onDataReceivedNoteDelgHandle, _logStateDEBUGDelgHandle))
                    {
                        msgLog = $@"{DateTime.Now:HH:mm:ss.fff_fff_1}; OpenPort - Start; COM Port: {COMPort}{"\r\n"}";
                        UpdateProcessState(GetDeviceInfoState.OpeningPort, "-Opening Port (01)~");
                        im30Port.OpenPort();
                        msgLog += $@"{DateTime.Now:HH:mm:ss.fff_fff_1}; OpenPort - End{"\r\n"}";

                        msgLog += $@"{DateTime.Now:HH:mm:ss.fff_fff_1}; Send Getting Reader Info command - Start{"\r\n"}";
                        UpdateProcessState(GetDeviceInfoState.SendingGetDeviceInfoCommand, "-Sending Get Reader Info command (A)~");
                        im30Port.WriteDataPort(dataResult, "-Sending Get Reader Info command (B)~");
                        msgLog += $@"{DateTime.Now:HH:mm:ss.fff_fff_1}; Send Getting Reader Info command - End{"\r\n"}";
                        UpdateProcessState(GetDeviceInfoState.ExpectingGetDeviceInfoCommandAck, "-Expecting Getting Reader Info Command Ack (AK01)~");

                        LogState(msgLog);
                        Log.LogText(_logChannel, _transactionID, msgLog, "K05", "IM30GetDeviceInfo.StartTransaction");
                        msgLog = "";
                        ...
                        while ((IsCurrentWorkingEnded == false) && (IsEndingProcess == false) && (IsDisposed == false) && (_processEndingStatesList?.Count > 0))
                        {
                            ...
                            {
                                UpdateProcessState(GetDeviceInfoState.Timeout, "-Reach Timeout (T1)~");
                            }
                            else
                            {
                                Thread.Sleep(20);
                ...
                {
                    transError = ex;
                    UpdateProcessState(GetDeviceInfoState.ErrorHalt, "-Error at Global Thread (T1)~");
                    LogState($@"'-Card reader Error; Fail Getting Reader Info~'; {ex.Message}; Last State : {_processState}; Process ID : {_transactionID}");
                    Log.LogError(_logChannel, _transactionID, new Exception($@"{ex.Message}; COM Port: {COMPort}; -Card reader Error; Fail Get Device Info~; Last State : {_processState}", ex), "EX10", "IM30GetDeviceInfo.StartTransaction");
                }
                finally
                {
                    // ----------------------------------------------------------
                    ...
                    if (FinalResult is null)
                    {
                        ...
                        {
                            if (_processState == GetDeviceInfoState.Busy)
                                FinalResult = new IM30GetDeviceInfoResult(new Exception($@"-Fail Getting Reader Info; Reader busy~"), null);
                            else
                                FinalResult = new IM30GetDeviceInfoResult(new Exception($@"-Fail Getting Reader Info; Unable to work properly; (A)~'; Last Process State : {_processState}"), null);

                        }
                        else
                            FinalResult = new IM30GetDeviceInfoResult(new Exception($@"-Fail Getting Reader Info; Unable to work properly; (B)~; Last Process State : {_processState}", transError), null);
                    }
                    // ----------------------------------------------------------
                    // Finalizing
                    ...
                    {
                        Log.LogError(_logChannel, _transactionID, new Exception($@"{ex2.Message}; COM Port: {COMPort}", ex2), "EX01", "IM30GetDeviceInfo.StartTransaction");
                    }
                    // ----------------------------------------------------------
                    LogState($@"--------- End Card Reader COM Port Sequences ---------; Process ID : {_transactionID}");
                    Log.LogText(_logChannel, _transactionID, $@"COM Port: {COMPort}; --------- End Card Reader COM Port Sequences : IM30GetDeviceInfo ---------", "H01", "IM30GetDeviceInfo.StartTransaction");
            ...
            byte[] ReadCOMPortData(IM30COMPort comPortX)
            {
                ...
                catch (Exception ex)
                {
                    if (retData?.Length > 0)
                    {
                        ...
                        Log.LogError(_logChannel, _transactionID, new Exception($@"{ex.Message}; COM Port: {COMPort}; Data Txt: {hisDataStr}", ex), "EX01", "IM30GetDeviceInfo.ReadCOMPortData");
                    }
        ...
        private void OnTransactionFinishedDelgWorking(IIM30TransResult finalResult)
        {
            ...
            Thread tWorker = new Thread(new ThreadStart(new Action(() =>
            {
                ...
                {
                    Log.LogError(_logChannel, _transactionID, new Exception($@"{ex.Message}; COM Port: {COMPort}", ex), "EX01", "IM30GetDeviceInfo.OnTransactionFinishedDelgWorking");
                }
                finally
                {
        ...
        private void ProcessResponseData(byte[] recData, IM30COMPort comPort)
        {
            ...
            try
            {
                ...
                Log.LogText(_logChannel, _transactionID, $@"COM Port: {COMPort}; Data Recv.Length: {recData.Length}", "A01", "IM30GetDeviceInfo.ProcessResponseData");
                //-----------------------------------------------------------------------------
                // Expect ACK
                if (recData.Length == 1)
                {
                    ...
                    if ((int)ASCIICodeEn.ACK == (int)recData[0]) /* ACK always for -> GetDeviceInfoState.ExpectingGetDeviceInfoCommandAck */
                    {
                        IsTransStartSuccessful = true;
                        LogState($@"Received Char : {dStr}");
                        Log.LogText(_logChannel, _transactionID, $@"COM Port: {COMPort}; Received Char : {dStr}", "B01", "IM30GetDeviceInfo.ProcessResponseData");
                        UpdateProcessState(GetDeviceInfoState.ExpectingFinalDeviceInfoResponse, "-Expecting card activities (CA1)~");
                    }
                    else if ((int)ASCIICodeEn.NAK == (int)recData[0]) 
                    {
                        LogState($@"-Card reader is busy~' Previous State :  {_processState}; Received Char : {dStr}");
                        Log.LogText(_logChannel, _transactionID, $@"COM Port: {COMPort}; -Card reader is busy~' Previous State :  {_processState}; Received Char : {dStr}", "B05", "IM30GetDeviceInfo.ProcessResponseData");
                        UpdateProcessState(GetDeviceInfoState.Busy, "-Busy Reading Found (D1)~");
                    }
                    else
                    {
                        LogState($@"-Unregconized card reader reading~' Char : {dStr}");
                        Log.LogText(_logChannel, _transactionID, $@"COM Port: {COMPort}; -Unregconized card reader reading~' Char : {dStr}", "B10", "IM30GetDeviceInfo.ProcessResponseData");
                    }
                }
                //------------------------------------------------------------------------------
                // Invalid Data Length
                else if (recData.Length < 5)
                {
                    ...
                    LogState($@"{dStr}");
                    Log.LogError(_logChannel, _transactionID, new Exception($@"COM Port: {COMPort}; {dStr}"), "X05", "IM30GetDeviceInfo.ProcessResponseData");
                }
                //------------------------------------------------------------------------------
                ...
                else
                {
                    ...
                    if (IM30RequestResponseDataWorks.ConvertToIM30DataModel(recData, out IM30DataModel im30DataMod, out Exception error2))
                    {
                        ...
                        //..im30DataMod always for GetDeviceInfoState.ExpectingFinalDeviceInfoResponse
                        FinalResult = new IM30GetDeviceInfoResult(im30DataMod);
                        IsPerfectCompleteEnd = true;

                        comPort.WriteDataPort(PortProtocalDef.ACKData, "ACK to Get Device Info Final Response");

                        UpdateProcessState(GetDeviceInfoState.Ending, "-Normal Ending (N1)~");
                    }
                    // Error Found --------------------------------------------------------------
                    else if (error2 != null)
                    {
                        FinalResult = new IM30GetDeviceInfoResult(error2, im30DataMod);
                        UpdateProcessState(GetDeviceInfoState.ErrorHalt, "-Data reading error (ED1)~");
                        LogState("-- Error --\r\n" + JsonConvert.SerializeObject(error2, Formatting.Indented));
                        Log.LogText(_logChannel, _transactionID, error2, $@"ED1#COM Port: {COMPort}", "IM30GetDeviceInfo.ProcessResponseData", AppDecorator.Log.MessageType.Error);
                    }
                    // Unknown Error ------------------------------------------------------------
                    else
                    {
...
Received Unknown Data (Text) : {System.Text.Encoding.ASCII.GetString(recData)}";
                        ...
                        Log.LogText(_logChannel, _transactionID, $@"COM Port: {COMPort}; {dStr}",
                            "X21", "IM30GetDeviceInfo.ProcessResponseData", AppDecorator.Log.MessageType.Error);

                        FinalResult = new IM30GetDeviceInfoResult(new Exception($@"-Unknown data reading error from card reader~ Last Process State : {_processState}"), null);
                        UpdateProcessState(GetDeviceInfoState.ErrorHalt, "-Unknown data reading error (ED2)~");
                        ...
                        Log.LogText(_logChannel, _transactionID, $@"COM Port: {COMPort}; " + "-- Unknown Error --\r\n" + "-Unknown error when reading data from card reader~",
                            "X22", "IM30GetDeviceInfo.ProcessResponseData", AppDecorator.Log.MessageType.Error);
                    }
                }
                //--------------------------------------------------------------------------------
            }
            ...
            {
                Log.LogError(_logChannel, _transactionID, new Exception($@"{ex.Message}; COM Port: {COMPort}", ex), "EX20", "IM30GetDeviceInfo.ProcessResponseData");
            }
        }
        ...
        private void UpdateProcessState(GetDeviceInfoState latestState, string locationTag = null)
        {
            if (IsDisposed)
                return;
            ...
            void UpdateProcessStateThreadWorking()
            {
                lock (_updateProcessStateLock)
                {
                    ...
                    else if (latestState > _processState)
                    {
                        ...
                        Log.LogText(_logChannel, _transactionID, $@"COM Port: {COMPort}; GetDeviceInfoState: {latestState}; Loct.:{locationTag}", "A01", "IM30GetDeviceInfo.UpdateProcessState");

                        if ((from stt in _processEndingStatesList
                             where stt == latestState
                             select stt).Count() > 0)
                        {
                            Log.LogText(_logChannel, _transactionID, $@"COM Port: {COMPort}; Ending State; Loct.:{locationTag}", "A03", "IM30GetDeviceInfo.UpdateProcessState");
                        }
                    }
                }
            }
                ...
                if (IsDisposed == false)
                {
                    if (IsCurrentWorkingEnded == false)
                    {
                        ...
                        Thread tWork = new Thread(new ThreadStart(new Action(() =>
                        {
                            if (IsCurrentWorkingEnded == false)
                            {
                                UpdateProcessState(GetDeviceInfoState.Ending, "-Shutdown Ending~");
                                Thread.Sleep(1000);
                                //_msg.ShowMessage("Request Stop Sale Transaction");
                            }
                        })))
        ...
        public static void Shutdown()
        {
            try
            {
                LastIM30GetDeviceInfoObj?.ShutdownX();
            }
            catch (Exception ex)
            {
                //_msg.ShowMessage(ex.ToString());
        ...
        public enum GetDeviceInfoState
        {
            New = 0,
            Start = 1,
            OpeningPort = 2,
            SendingGetDeviceInfoCommand = 3,
            ExpectingGetDeviceInfoCommandAck = 4,
            ExpectingFinalDeviceInfoResponse = 6,
