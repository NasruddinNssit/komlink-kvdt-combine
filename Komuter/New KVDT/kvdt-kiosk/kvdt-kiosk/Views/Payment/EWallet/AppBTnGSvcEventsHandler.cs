...
namespace kvdt_kiosk.Views.Payment.EWallet
{
    public class AppBTnGSvcEventsHandler
    {
        ...
        private void BTnGService_OnDataReceived(object sender, DataReceivedEventArgs e)
        {
            ...
            {
                if(App._purchaseTicketScreen != null)
                {
                    App._purchaseTicketScreen?.BTnGShowPaymentInfo(e.ReceivedData.MsgObject);

                }
                else if(App.komlinkCardDetailScreen != null)
                {
                    App.komlinkCardDetailScreen?.BTnGShowPaymentInfo(e.ReceivedData.MsgObject);
                }
                else
                {
                    App._parcelLayout?.BTnGShowPaymentInfo(e.ReceivedData.MsgObject);
                }
            }
        }
    }
