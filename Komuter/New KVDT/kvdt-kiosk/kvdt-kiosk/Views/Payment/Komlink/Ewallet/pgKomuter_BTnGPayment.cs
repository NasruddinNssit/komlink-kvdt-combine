...
namespace kvdt_kiosk.Views.Payment.Komlink.Ewallet
{
    public class pgKomuter_BTnGPayment
    {
        ...
        public pgKomuter_BTnGPayment(Frame frmSubFrame, Border bdSubFrame) 
        { 
        ...
        private void _bTnGPaymentPage_OnEndPayment(object sender, kvdt_kiosk.Views.Payment.Komlink.PayWave.EndOfPaymentEventArgs e)
        {
            try
            {
            ...
            void OnEndPaymentThreadWorking()
            {
                try
                {
                    ...
                    {
                        App.komlinkCardDetailScreen.CheckOutTopUp();
                    }
                    else if ((e.ResultState == NssIT.Kiosk.AppDecorator.Common.PaymentResult.Cancel) || (e.ResultState == NssIT.Kiosk.AppDecorator.Common.PaymentResult.Fail))
                    {
                        ...
                        App.komlinkCardDetailScreen.Dispatcher.Invoke(new Action(() =>
                        {
                            _bTnGPaymentPage.ClearEvents();
                            _frmSubFrame.Content = null;
                            _frmSubFrame.NavigationService.RemoveBackEntry();
