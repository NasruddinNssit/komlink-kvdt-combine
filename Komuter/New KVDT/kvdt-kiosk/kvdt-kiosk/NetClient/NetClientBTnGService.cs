...
namespace kvdt_kiosk.NetClient
{
    public class NetClientBTnGService
    {
        ...
        {
            var param = System.IO.File.ReadAllText(@"C:\NssITKiosk\LocalServer\Parameter.txt");

            var localServerPort = param.Split(new[] { "\r\n", "\r", "\n" }, StringSplitOptions.None)
                .Where(x => x.StartsWith("LocalServerPort=")).FirstOrDefault();
            
            localServerPort = localServerPort.Substring(localServerPort.IndexOf('=') + 1);
            return int.Parse(localServerPort);
        }
